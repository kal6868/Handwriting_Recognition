...
rgb = cv2.imread('./hello2.jpg', cv2.IMREAD_GRAYSCALE)
cv2.imshow('grey', rgb)

ret, binary = cv2.threshold(rgb, 240, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
cv2.imshow('binary', binary)

kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (13, 13)) # 최적 7. 9
binary = cv2.morphologyEx(rgb, cv2.MORPH_GRADIENT, kernel)
cv2.imshow('binary', binary)

binary2 = cv2.morphologyEx(binary, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3,3)), iterations = 2)
cv2.imshow('binary2', binary2)

kernel2 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 4)) # # 최적 4. 1
connected = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel2)
cv2.imshow('connected', connected)

blur = cv2.GaussianBlur(connected, (9,9), 0)

contours , hierarchy = cv2.findContours(blur , cv2.RETR_EXTERNAL , cv2.CHAIN_APPROX_SIMPLE) #외곽선 검출
color = cv2.cvtColor(blur, cv2.COLOR_GRAY2BGR) #이진화 이미지를 color이미지로 복사(확인용)
...
for x,y,w,h in bR_arr:
    if w and h > 100:
        cv2.rectangle(rgb,(x-2,y-2),(x+w+2,y+h+2),(0,0,255),1)
        img_result.append(img_for_class[y:y+h, x:x+w])

cv2.imshow('boxing', rgb)
cv2.imshow('one', img_result[3])
...
for n in img_result:
    ...
    plt.imshow(cv2.resize(n,(32,32)), cmap='Greys', interpolation='nearest')
plt.tight_layout()
plt.show()
...
def load():
    ...
    saved_model = './saved_model/25_epoch_model.h5'
...
# 각 손글씨를 한 자씩 예측
cv2.cvtColor(img_result[0], cv2.COLOR_GRAY2RGB).shape
img_resize = []
for i in range(0, len(img_result)):
    res = cv2.resize(img_result[i], (32, 32))
    cvt = cv2.cvtColor(res, cv2.COLOR_GRAY2RGB)
    img_resize.append(cvt)
...
for i in range(0, len(img_resize)):
    img_resize_array = np.array(img_resize[i], 'uint8')
    T_img_resize_array = img_resize_array.reshape((1, 32, 32, 3))
    a = model.predict(T_img_resize_array)
    b = np.argmax(a, axis=1)
    hdw_recog.append(idx2char[(b[0])])